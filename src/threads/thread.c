...
void calculate_priority(struct thread *cur)
{
  ...
  cur->priority = PRI_MAX - TO_NEARINT(cur->recent_cpu.val / 4) - (cur->nice * 2);
...
void Calculate_recent_cpu_all(void)
{
  ...
  while (e != list_end(&all_list))
  {
    t = list_entry(e, struct thread, allelem);
    struct real temp;
    temp.val = DIV_REAL((2 * load_avg.val), ADD_REAL_INT((2 * load_avg.val), 1));
    t->recent_cpu.val = ADD_REAL_INT(MUL_REAL(temp.val, t->recent_cpu.val), t->nice);
    e = list_next(e);
  }
  // sort_thread_list (&ready_list);
}
...
void Calculate_load_avg(void)
{
  ...
  load_avg.val = MUL_REAL(TO_REAL(59) / 60, load_avg.val) + (TO_REAL(1) / 60) * ready_threads;
}

bool list_less_comp(const struct list_elem *a, const struct list_elem *b, void *aux UNUSED)
{
  const int a_member = (list_entry(a, struct thread, sleepelem))->wake_time;
  const int b_member = (list_entry(b, struct thread, sleepelem))->wake_time;
...
{
  const int a_member = (list_entry(a, struct lock, elem))->priority;
  const int b_member = (list_entry(b, struct lock, elem))->priority;
  return a_member > b_member;
}
...
void thread_yield(void)
{
  ...
  old_level = intr_disable();
  if (cur != idle_thread) // idle thread is not included in the ready_list
...
void thread_set_virtual_priority(struct thread *t, int new_priority)
{
...
void thread_set_nice(int nice) // UNUSED
{
  ...
  struct real temp;
  temp.val = DIV_REAL((2 * load_avg.val), ADD_REAL_INT((2 * load_avg.val), 1));
  thread_current()->recent_cpu.val = ADD_REAL_INT(MUL_REAL(temp.val, thread_current()->recent_cpu.val), nice);
...
{
  return TO_NEARINT(100 * (load_avg.val));
}

/* Returns 100 times the current thread's recent_cpu value. */
int thread_get_recent_cpu(void)
{
  return TO_NEARINT(100 * (thread_current()->recent_cpu.val));
}
